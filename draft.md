...
Pour la première raison, éviter l’effet tunnel est possible, pour les deux dernières c’est souhaitable. On peut, dans le développement logiciel, se permettre de développer morceau par morceau en montrant les résultats au client au fur et à mesure. Chaque correctif est une dépense de temps mais elle reste moindre que si un grand retour en arrière est nécessaire au moment de la livraison.

C’est pour ces raison que les méthodologies agiles, qui se basent sur des livraisons courtes et qui intègrent les correctifs continus, ont le vent en poupe. C’est également pour ces raisons que le monde du développement n’arrivera jamais à basculer entièrement vers un mode de fonctionnement en *one shot*. C’est pour ces raisons que le pôle SAP continue à travailler de manière moins formelle pour certaines demandes. Enfin, c’est pour ces raison que le système de gestion de tâches créé par le pôle étude, bien qu’envisagé en tant que gestion de projet “à la” cycle en V, se rapproche en réalité plus d’une gestion agile des tâches sans gestion de sprint.

Les tâches sur le worflow comptabilité furent une bonne manière d’expérimenter plusieurs de ces points. Le workflow fut l’objet de plusieurs demandes, demandes qui étaient en réalité dans une certaine continuité entre-elles. Il est tout à fait possible d’imaginer que ces demandes euent été séparées de manière encore plus fine. Aussi, chaque demande faisait l’objet de nombreux aller-retours entre le développeur (moi) et le chef de projet, montrant ainsi l’aspect nécessaire du travail par itération.
Mis à part les gros projets apportant de nouveaux flux et impliquant plusieurs services, et où un travail considérable en amont est absolument indispensable, on peut très bien imaginer qu’un travail itératif assumé soit tout à fait profitable au pôle SAP pour beaucoup de tâche. En effet une quantité importante de tâche concerne soit une évolution, soit de la maintenance. Dans ces cas un travail à cycle court et répété, avec des feedbacks nombreux, est idéal.

Quoi qu’il en soit, au delà de l’organisation du pôle, un travail par itération et en faisant fonctionner le feedback fut particulièrement pour ces tâches, ne serait-ce que pour la partie de travail individuelle.
...
#### Gestion des risques
Cette tâche fut éprouvante vis à vis de la gestion des risques. En effet, lors de la seconde confrontation avec l’extraction FI pour Cognos, il a fallu retravailler la façon dont plusieurs mécanismes étaient mis en œuvre. Tentons de voir très rapidemment quel était le problème à régler.

Nous avons vu que les transactions d’extraction FI pour Cognos concernaient les offres pour FI. Les offres sont quelque chose d’essentiel au flux NOZ et leur gestion est intégré dans SAP depuis un certain temps. Les fonctionnalités en plus qui ont été ajoutées pour les offres FI venaient donc se greffer à des fonctionnalités existantes. Il a été à ce moment là décidé que les données FI devraient être intégrées à la table principale des offres. Chaque entrée de table contenant les informations de base des offres contiendrait aussi les données FI. L’inconvénient à cela était une certaine perte de place en base de données (des offres non-FI auraient des champs systématiquement vides et donc inutiles, mais qui occuperaient quand même de la place). Les avantages étaient en revanche un certain gain de performance d’accès (toutes les infos FI sont alors pour ainsi dire “à portée de main”) et une grande simplicité de mise en place. Des champs furent donc ajoutées à la table des offres.

Un autre problème potentiel avait aussi été identifié : une offre ne peut être éditée qu’à un seul endroit à la fois. Si un utilisateur veut éditer des informations FI sur une offre alors qu’un autre utilisateur est déjà en train d’éditer d’autres informations (sans rapport nécessaire avec FI), il va se faire rejeté par le système. Une entrée de table est très généralement bloquée lorsqu’un utilisateur se met à l’éditer (on lui met un verrou), les accès concurrents sont donc impossibles. La solution que nous avons choisi qui consiste à mettre les données générales des offres *et* les données spécifiques à FI entrainerait donc une impossibilité de concurrence pour les éditions d’offres dans SAP. Les chances d’avoir deux utilisateurs voulant tous deux éditer une même offre en même temps fut jugé suffisament faible et c’est pourquoi cette solution fut choisie malgré le risque. C’est malheureusement ce qui a fini par se produire à de nombreuses reprisis. Il a donc fallu retravailler les transactions d’extrations.

La tâche fut compliquée car elle se devait d’être transparente et, évidemment, sans entrainer aucune régression. Il fallait :
* extraire les données de la table des offres et les mettre dans une table qui ne contiendrait que les données FI
* modifier la transaction d’édition des données générales d’offre : lui faire oublier les champs FI
* modifier la transaction des données spécifiques FI : lui faire regarder dans la nouvelle table spécifique FI
* modifier le programme d’extraction des données vers Cognos pour qu’il prenne en compte les nouvelles données
Le tout à la manière d’un réaiguillage de train (en route).
